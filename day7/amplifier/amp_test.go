...
func TestCircuitNoFeedback(t *testing.T) {
	tt := []struct {
		...
		{
			instructions: []int{3, 15, 3, 16, 1002, 16, 10, 16, 1, 16, 15,
				15, 4, 15, 99, 0, 0},
			ps:       []int{4, 3, 2, 1, 0},
			expected: 43210,
		},
	...
	for _, tc := range tt {
		...
		c := CreateAmpCircuit(n, tc.ps, tc.instructions, log, false)
...
func TestCircuitFeedback(t *testing.T) {
	tt := []struct {
		...
		{
			instructions: []int{3, 26, 1001, 26, -4, 26, 3, 27, 1002, 27, 2,
				27, 1, 27, 26, 27, 4, 27, 1001, 28, -1, 28, 1005, 28, 6, 99, 0,
				0, 5},
			ps:       []int{9, 8, 7, 6, 5},
			expected: 139629729,
		},
		{
			instructions: []int{3, 52, 1001, 52, -5, 52, 3, 53, 1, 52, 56,
				54, 1007, 54, 5, 55, 1005, 55, 26, 1001, 54, -5, 54, 1105, 1,
				12, 1, 53, 54, 53, 1008, 54, 0, 55, 1001, 55, 1, 55, 2, 53,
	...
	for _, tc := range tt {
		log := intcomputer.CreateLogger()
		c := CreateAmpCircuit(n, tc.ps, tc.instructions, log, true)
